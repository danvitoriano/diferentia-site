---
title: Readme
---

# Workshop: Claude Cowork (prático)

**Produto:** Diferentia  
**Carga horária:** 3h  
**Formato:** Laboratório: presencial ou online síncrono  
**Trilha:** [cursos-oceana](https://github.com/diferentia/cursos-oceana): **workshop opcional**

---

## Sobre este workshop

Cada participante executa ao vivo no **app desktop** do Claude. Ao final, o ambiente deve estar funcionando: pasta conectada, pelo menos um plugin instalado, skill testada e tarefa agendada configurada.

Não substitui o [Cowork demonstrativo](https://github.com/diferentia/curso-cowork) nem o [curso introdutório](https://github.com/diferentia/curso-claude-introdutorio).

---

## Pré-requisitos

- [Curso Introdutório](https://github.com/diferentia/curso-claude-introdutorio) concluído (recomendado).
- [Cowork demonstrativo](https://github.com/diferentia/curso-cowork) (recomendado: contexto e decisão sobre Pro).
- **Licença Claude Pro ativa** e app desktop instalado **antes** da aula.
- E-mail de preparação do facilitador com **72h de antecedência**.

---

## Roteiro

**Arquivo:** [`cowork-workshop-roteiro.md`](cowork-workshop-roteiro.md)

---

## Posição na trilha

| Camada | Entrega |
|---|---|
| Núcleo | Introdutório → Cowork demonstrativo |
| **Este repo** | Workshop Cowork (opcional) |
| Opcional | [Claude Code](https://github.com/diferentia/curso-claude-code) · [Avançado](https://github.com/diferentia/curso-claude-code-avancado) |

---

## Sobre a Diferentia

[differentia.com.br](https://differentia.com.br)
